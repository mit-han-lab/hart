...
__global__ void fused_rope_forward_kernel(
  ...
  fused_rope_block_forward<scalar_t>(src, freqs, dst, offset_block,
...
void fused_rope_forward_launcher(const scalar_t *input, const float *freqs,
  ...
  fused_rope_forward_kernel<scalar_t><<<blocks, threads, 0, stream>>>(
...
template <typename scalar_t>
void fused_rope_forward(const at::Tensor &input, const at::Tensor &freqs,
  ...
  fused_rope_forward_launcher<scalar_t>(
...
template <typename scalar_t>
void fused_rope_backward(const at::Tensor &output_grads,
  ...
  fused_rope_backward_launcher<scalar_t>(
      reinterpret_cast<const scalar_t *>(output_grads.data_ptr()),
      reinterpret_cast<const float *>(freqs.data_ptr()),
      reinterpret_cast<scalar_t *>(input_grads.data_ptr()), s, b, h, d, d2,
...
template <typename scalar_t>
void nvte_fused_rope_forward(const at::Tensor input, const at::Tensor freqs,
                             at::Tensor output, const int s, const int b,
                             const int h, const int d, const int d2,
                             const int stride_s, const int stride_b,
                             const int stride_h, const int stride_d,
  ...
  fused_rope_forward<scalar_t>(input, freqs, output, s, b, h, d, d2, stride_s,
                               stride_b, stride_h, stride_d, o_stride_s,
                               o_stride_b, o_stride_h, o_stride_d, stream);
}

template <typename scalar_t>
void nvte_fused_rope_backward(const at::Tensor output_grads,
                              const at::Tensor freqs, at::Tensor input_grads,
                              const int s, const int b, const int h,
                              const int d, const int d2, const int stride_s,
                              const int stride_b, const int stride_h,
  ...
  fused_rope_backward<scalar_t>(output_grads, freqs, input_grads, s, b, h, d,
                                d2, stride_s, stride_b, stride_h, stride_d,
                                o_stride_s, o_stride_b, o_stride_h, o_stride_d,
                                stream);
}
...
at::Tensor fused_rope_forward_func(const at::Tensor &input,
  ...
  VLLM_DISPATCH_FLOATING_TYPES(
      input.scalar_type(), "nvte_fused_rope_forward", [&] {
        nvte_fused_rope_forward<scalar_t>(
...
at::Tensor fused_rope_backward_func(const at::Tensor &output_grads,
  ...
  VLLM_DISPATCH_FLOATING_TYPES(
      output_grads.scalar_type(), "nvte_fused_rope_forward", [&] {
        nvte_fused_rope_backward<scalar_t>(
            output_grads_cu.data(), freqs_cu.data(), input_grads_cu.data(), s,
            b, h, d, d2, stride_s, stride_b, stride_h, stride_d, o_stride_s,
            o_stride_b, o_stride_h, o_stride_d,
            at::cuda::getCurrentCUDAStream());
      });
